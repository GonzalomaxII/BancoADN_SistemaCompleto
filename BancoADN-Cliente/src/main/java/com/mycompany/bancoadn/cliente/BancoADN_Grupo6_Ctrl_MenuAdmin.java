 ...
 * ── NOVEDADES ─────────────────────────────────────────────────────────────────
 *
 *   btnUltimasSolicitudes  → abre Pant_UltimasSolicitudes con Ctrl_UltimasSolicitudes
 *   btnAdministrarPerfiles → abre Pant_AdminPerfiles       con Ctrl_AdminPerfiles
 *
 * ── PROTOCOLO SOCKET ──────────────────────────────────────────────────────────
 *
 *   LISTAR       → "ListaSol"
 *                  recibe líneas con enviarYSolicitarLista hasta "FINISH":
 *                  [0]idSolicitud - [1]tipo - [2]estado - [3]datosSolicitud
 *                  - [4]IdAdmin - [5]IdUsuario - [6]idPerfil
 *                  - [7]fechaCreación - [8]fechaResolución
 *
 *   EMAIL PERFIL → "EmailPorPerfil - <idPerfil>"
 *                  recibe una línea: email del dueño del perfil, o "—"
 *
 *   RESOLVER     → "ResSol - <idSolicitud> - <estadoNuevo> - <emailAdmin>"
 *                  "Aceptado con exito" / "rechazado con exito" / "No existe..."
 *
 *   ÚLTIMAS SOL  → "UltSol"   (manejado en Ctrl_UltimasSolicitudes)
 *
 *   ADMIN PERF   → "BuscarPerfilesAdmin - ..." / "BajaAdmin - ..." / ...
 *                  (manejados en Ctrl_AdminPerfiles)
 */
public class BancoADN_Grupo6_Ctrl_MenuAdmin {
    ...
    public BancoADN_Grupo6_Ctrl_MenuAdmin(BancoADN_Grupo6_MenuAdmin vista, ClienteSocket clienteSocket, String emailAdmin) {
        this.vista         = vista;
    ...
    private void cargarSolicitudes() {
        ...
        for (String linea : lineas) {
            ...
            if (tipo.equals("registrar") || tipo.equals("modificar")) {
                // tokens[0] = email del solicitante, tokens[3] = descripción
                mailMostrar = tokens.length > 0 ? tokens[0].trim() : "—";
                lineaExtra  = tokens.length > 3 ? tokens[3].trim() : "—";
            } else {
                // baja/restaurar: datosSolicitud llega vacío;
                // el email del titular se obtiene por idPerfil y se muestra en línea 1 Y línea 3
    ...
    private void manejarCerrarSesion() {
        ...
        if (opc == JOptionPane.YES_OPTION) {
            vista.dispose();
            Pant_IniciarSesion loginVista = new Pant_IniciarSesion();
